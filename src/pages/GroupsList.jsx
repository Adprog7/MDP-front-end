...
const GroupsList = () => {
  ...
  useEffect(() => {
    // Remplacer par ton endpoint : /api/messages ou /api/conversations
  ...
  return (
    <div className="min-h-screen bg-[#FDFBF7] font-sans antialiased relative overflow-hidden pb-28">
      ...
      <div className="relative z-10 px-5 flex flex-col gap-6">
        {loading ? (
          <p className="text-center text-gray-400 font-bold py-10">Chargement des messages...</p>
          ...
          filteredConversations.map((msg) => (
            ...
            >
              <img src={msg.image} alt={msg.name} className="w-[60px] h-[60px] rounded-full object-cover shadow-sm shrink-0" />
              <div className="flex flex-col justify-center min-w-0">
                <h3 className="text-[15px] font-black text-gray-900 truncate leading-tight mb-0.5">{msg.name}</h3>
                <p className="text-[13px] text-gray-500 font-medium truncate">{msg.lastMsg}</p>
              </div>
            </Link>
          ))
        ) : (
          <p className="text-center text-gray-400 font-bold py-10">Aucune conversation trouvée.</p>
        )}
      </div>
